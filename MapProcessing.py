...
LONG_PER_GRID = 2 * np.pi / NUM_COLUMNS  # vertical (radians)
LAT_PER_GRID = np.pi / NUM_ROWS  # horizontal (radians)
LONG_PER_PIXEL = LONG_PER_GRID / COLUMN_SIZE
LAT_PER_PIXEL = LONG_PER_GRID / ROW_SIZE
...
def lat_long_to_row_col(lat: float, long: float) -> tuple[int, int]:
    ...
    row = (lat / LAT_PER_PIXEL) + IM_HEIGHT // 2
    col = (long / LONG_PER_PIXEL) + IM_WIDTH // 2
...
def test():
    arr = image_to_numpy_array("WorldMapTwo.jpg")
